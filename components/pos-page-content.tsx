    ...
    <AppHeader businessName={membership.businessName} outletName={activeOutlet.name} outlets={outlets} activeOutletId={activeOutlet.id} role={membership.role} trialDaysRemaining={subDetails.isTrialing ? subDetails.daysRemaining : null}>
      <section className="mx-auto w-[min(1400px,calc(100%-24px))] py-5 animate-page-enter sm:w-[min(1400px,calc(100%-40px))] sm:py-6">
        <header className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
          <div><h1 className="text-2xl font-bold tracking-[-0.7px] text-[#17211d]">Kasir</h1><p className="mt-1 text-sm text-[#6c7a73]">Cari produk, cek pesanan, lalu selesaikan pembayaran.</p></div>
        ...
        <details className="group mt-4 border border-[#d9e2dd] bg-white">
          <summary className="flex cursor-pointer list-none items-center justify-between px-4 py-3 text-sm font-semibold text-[#33423b] [&::-webkit-details-marker]:hidden">
            <span>Transaksi terakhir</span><span className="text-xs font-normal text-[#78857f] group-open:hidden">Lihat {recentSales.length} transaksi</span><span className="hidden text-xs font-normal text-[#78857f] group-open:inline">Tutup daftar</span>
          </summary>
              ...
              <tbody>
                {recentSales.map((item) => (
                  <tr key={item.invoiceNumber} className={`border-b border-[#f0f4f1] transition ${item.status === "voided" ? "bg-rose-50/40 opacity-75" : ""}`}>
                    <td className="px-4 py-3 font-semibold"><div className="flex items-center gap-2"><Link href={`/sales/${item.id}`} className={`underline ${item.status === "voided" ? "text-rose-600 line-through" : "text-[#198760]"}`}>{item.invoiceNumber}</Link>{item.status === "voided" && <span className="rounded bg-rose-100 px-1.5 py-0.5 text-[10px] font-extrabold text-rose-700">VOID</span>}</div></td>
                    <td className="px-4 py-3 text-[#627069]">{new Date(item.createdAt).toLocaleString("id-ID")}</td><td className="px-4 py-3 capitalize">{item.paymentMethod}</td><td className={`px-4 py-3 text-right font-bold ${item.status === "voided" ? "text-rose-600 line-through" : ""}`}>Rp {Number(item.total).toLocaleString("id-ID")}</td>
                  </tr>
                ))}
