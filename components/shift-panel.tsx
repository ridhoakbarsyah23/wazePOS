...
export function ShiftPanel({ outlets, currentShift }: { outlets: Outlet[]; currentShift: Shift }) {
  ...
  return (
    <section className="rounded-2xl border border-[#dfe8e3] bg-white p-6 shadow-[0_8px_24px_rgba(16,65,48,.06)]">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-lg font-extrabold">Shift kasir</h2><p className="mt-1 text-sm text-[#627069]">{shift ? `Terbuka sejak ${new Date(shift.openedAt).toLocaleString("id-ID")}` : "Buka shift sebelum mulai operasional kasir."}</p></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${shift ? "bg-[#eaf7f0] text-[#198760]" : "bg-[#fff0e5] text-[#a35f12]"}`}>{shift ? "Shift terbuka" : "Belum ada shift"}</span></div>
